...
def test_depth():
    ...
    depth_coeff = numpy.ones(shape = disparity_reference.shape) * (3979.911 * 193.001) # Baseline * focal
...
def save_in_24_bits(data):
    #   R             G                B
    # x % 255   x << 8 % 255    x << 16 % 255
    disparity_map_normalized = get_normalized(data)

    disparity_map_24bit_range = numpy.multiply(disparity_map_normalized, 16777216).astype(numpy.uint32) # max 24bit value
    disparity_coded = numpy.ones(shape=(data.shape[0], data.shape[1], 3), dtype=numpy.uint8)
    
    r_channel = (disparity_map_24bit_range & 0xFF).astype(numpy.uint8)
    g_channel = ((disparity_map_24bit_range >> 8) & 0xFF).astype(numpy.uint8)
    b_channel = ((disparity_map_24bit_range >> 16) & 0xFF).astype(numpy.uint8)
    disparity_coded[:,:,0] = r_channel
    disparity_coded[:,:,1] = g_channel
    disparity_coded[:,:,2] = b_channel

    return disparity_coded

def load_from_24_bits(data):
    r_channel = data[:,:,0].astype(numpy.uint8)
    g_channel = data[:,:,1].astype(numpy.uint8)
    b_channel = data[:,:,2].astype(numpy.uint8)

    loaded_data = numpy.zeros(dtype=numpy.uint32, shape=r_channel.shape)
    loaded_data = (r_channel.astype(numpy.uint32) & 0xFF)
    loaded_data += (g_channel.astype(numpy.uint32) << 8)
    loaded_data += (b_channel.astype(numpy.uint32) << 16)
    
    return loaded_data

def get_depth(disparity, baseline, focal):
    # what is this for?
    #disparity = disparity_normalized.astype(numpy.float32) / 16

    disparity = get_normalized(disparity)
    disparity_normalized = numpy.add(disparity, 0.00001)

    depth_coeff = numpy.ones(shape = disparity_normalized.shape) * (baseline * focal)
    depth = numpy.divide(depth_coeff, disparity_normalized)

    return depth

def exam_initial():
    # load the images, find the disparity map
    image_scale = 0.4
    ...
    left_img.rgb_data = cv2.resize(left_img.rgb_data, (0, 0), fx=image_scale, fy=image_scale)
    right_img.rgb_data = cv2.resize(right_img.rgb_data, (0, 0), fx=image_scale, fy=image_scale)

    searcher = DisparitySearch(".\\Motorcycle-perfect\\motor_L.png", ".\\Motorcycle-perfect\\motor_R.png", image_scale)
    disparity_custom = searcher.find_disparity_map()

    print(disparity_custom.max())
    #cv2.imshow("disparity calculated", disparity_custom)

    disparity_coded = save_in_24_bits(disparity_custom)
    cv2.imwrite('disparity_custom_channel_coded.jpg', disparity_coded, [cv2.IMWRITE_JPEG_QUALITY, 100])

    read_values = cv2.imread('disparity_custom_channel_coded.jpg')

    disparity_map_normalized = get_normalized(disparity_custom)
    disparity_loaded = load_from_24_bits(read_values)
    disparity_loaded_normalized = get_normalized(disparity_loaded)
    error_bm = numpy.mean(numpy.subtract(disparity_map_normalized, disparity_loaded_normalized) ** 2)
    print(error_bm)

    cv2.imshow("disparity ref", numpy.multiply(disparity_map_normalized, 255).astype(numpy.uint8))
    cv2.imshow("disparity loaded", numpy.multiply(disparity_loaded_normalized, 255).astype(numpy.uint8))
    cv2.waitKey()

    depth_map_original = get_depth(disparity_custom, 3979.911, 193.001)
    depth_map_loaded = get_depth(disparity_loaded, 3979.911, 193.001)

    cv2.imshow("depth loaded", numpy.multiply(get_normalized(depth_map_loaded), 255).astype(numpy.uint8))
    cv2.imshow("depth original", numpy.multiply(get_normalized(depth_map_original), 255).astype(numpy.uint8))
    cv2.waitKey()

    coded_depth_map = save_in_24_bits(depth_map_loaded)
    cv2.imwrite('depth_custom_channel_coded.jpg', coded_depth_map, [cv2.IMWRITE_JPEG_QUALITY, 100])
